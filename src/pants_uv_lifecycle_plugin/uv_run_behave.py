...
async def uv_run_behave_on_pyproject_modules(
    feature_file_targets: FeatureFileTargets,
) -> UvRunBehaveGoal:
    path_environment_variable = await Get(EnvironmentVars, PathEnvVariableRequest())

    feature_filepaths = await Get(
        FeatureFilepaths, FeatureFileTargets, feature_file_targets
    )
    ...
    logger.info(
        "`uv run behave` successfully completed for all modules with behave tests."
    )
